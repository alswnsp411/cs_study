...
통신 양단에 있는 최종 사용자 사이의 연결을 제공하여 목적지 호스트까지 신뢰할 수 있는 데이터가 도달할 수 있도록 한다. **어떤 식의 통신**을 할 것인가 결정하는 계층. 실제로 보내려는 데이터가 어떻게 목적지까지 찾아가게 만들것인가는 결정하지 않는다(이것은 네트워크 계층).
...
✨ 카톡 전송을 통해 두 호스트 간 데이터 통신 알아보기 !  

 ![데이터 전송 과정](https://github.com/kksshh0612/cs_study/assets/74577811/7eeca275-9c59-4459-b62d-0962491aba50)  
 
 - 송신 호스트에서 카톡으로 "안녕" 이라는 메시지를 보낸다.
   송신 호스트의 각 계층에서는 맡은 역할에 따라 프로토콜을 처리하고, 상위 계층으로부터 받은 데이터에 헤더를 붙여 하위 계층으로 넘긴다.  > 캡슐화
 - 응용 계층 (5, 6, 7 계층)
데이터 전송 과정 : https://better-together.tistory.com/89


### Transport Layer와, Network Layer의 차이에 대해 설명해 주세요.
4계층인 Transport Layer는 통신 시 어떤 식의 통신을 할 것인지 결정하는 역할을 한다.
3계층인 Network Layer는
